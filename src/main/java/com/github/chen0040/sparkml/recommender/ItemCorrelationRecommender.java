...
public class ItemCorrelationRecommender {
   ...
   public JavaRDD<ItemCorrelation> fitAndTransform(JavaRDD<UserItemRating> cells){
      ...
      JavaPairRDD<Tuple2<String, String>, Tuple7<Double, Double, Integer, Integer, Double, Double, Double>> rdd5 = rdd4.flatMapToPair(s -> {
            String user = s._1();
            Iterable<Tuple3<String, Double, Integer>> user_ratings = s._2();
            List<Tuple3<String, Double, Integer>> values = new ArrayList<>();
            for(Tuple3<String, Double, Integer> entry : user_ratings) {
               values.add(entry);
            }
            ...
            for(int i=0; i < values.size()-1; ++i) {
               Tuple3<String, Double, Integer> user_rating_i = values.get(i);
               String movie1 = user_rating_i._1();
               double rating1 = user_rating_i._2();
               int numRater1 = user_rating_i._3();

               for(int j=i+1; j < values.size(); ++j) {
                  Tuple3<String, Double, Integer> user_rating_j = values.get(j);
